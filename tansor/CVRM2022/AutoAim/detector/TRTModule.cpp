...
TRTModule::TRTModule(const std::string &onnx_file) {
    ...
    if (true) {
        build_engine_from_cache(onnx_file.c_str());
    } else {
        build_engine_from_onnx(onnx_file.c_str());
        std::cout << "build_engine_from_onnx finished" << std::endl;
        cache_engine("/home/hfut/tansor/CVRM2022/asset/model.cache");
        std::cout << "cache_engine finished" << std::endl;
    }
...
void TRTModule::build_engine_from_onnx(const std::string &onnx_file) {
    ...
    std::cout << "[INFO]: created Network" << std::endl;
    ...
    config->setMaxWorkspaceSize(free/3*2);
    ...
    //TRT_ASSERT((engine = static_cast<ICudaEngine *>(builder->buildSerializedNetwork(*network, *config)->data())) != nullptr);
    delete config;
    delete parser;
    delete network;
    delete builder;
...
void TRTModule::cache_engine(const std::string &cache_file) {
    //IHostMemory *engine_buffer = engine->serialize();
    //std::cout << (engine_buffer!=nullptr)  << std::endl;
    TRT_ASSERT(serializedModel != nullptr);
    std::cout << "serialized" << std::endl;
    std::ofstream ofs(cache_file, std::ios::binary);
    ofs.write(static_cast<const char *>(serializedModel->data()), serializedModel->size());
...
std::vector<bbox_t> TRTModule::operator()(const cv::Mat &src) const {
    ...
    cv::Mat x;
    float fx = (float) src.cols / 640.f, fy = (float) src.rows / 480.f;
    cv::cvtColor(src, x, cv::COLOR_BGR2RGB);
    if (src.cols != 640 || src.rows != 480) {
        cv::resize(x, x, {640, 480});
    }
