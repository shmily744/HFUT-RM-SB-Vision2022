...
[[noreturn]] void AutoAim::detection_run(const std::string &onnx_file)
{
    ...
    if (debug)
        std::cout << "============ detection_run ===========" << std::endl;
    ...
    KF::MeasurementStateConversionMatrix H =
        (KF::MeasurementStateConversionMatrix() << 1, 0, 0, 0,
         0, 1, 0, 0)
            .finished();

    // process noise covariance
    KF::StateMatrix Q = 4. *
                        (KF::StateMatrix() << 1, .5, 0, 0,
                         .5, 1, 0, 0,
                         0, 0, 1, .5,
                         0, 0, .5, 1)
                            .finished();
    ...
    while (true)
    {
        ...
        static int count = 0;
        static string write_buffer3 = "A0000 000B";
        ...
        if (true)
        {
            ...
            {
                //&& b.tag_id!=2
                if (b.color_id == ENEMY && b.tag_id != 0 && b.tag_id != 6 && b.tag_id != 5)
                { // enemy 0 blue 1 red    //不打2号工程
            ...
            if (Max_area)
            {
                ...
                write_buffer3 = "A1" + num(to_string(p_p.x)) + " " + num(to_string(p_p.y)) + "B";
                if (p_p.x < 0 || p_p.y < 0 || p_p.x > 640 || p_p.y > 480)
                {
                    string write_buffer2 = "A1" + num(to_string(X_c)) + " " + num(to_string(Y_c)) + "B";
                    write_buffer3 = write_buffer2;
            ...
            else
            {
                ...
                {
                    cout << count << endl;
                    cout << write_buffer3 << endl;
                    int bytes_written = write(fd, write_buffer3.c_str(), write_buffer3.size());
                    count = count + 1;
                }
                else
                {
                    count = 0;
                    write_buffer3 = "A0000 000B";
